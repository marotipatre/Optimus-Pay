...
import { getAndValidateRequestData } from "@/lib/functions/validate-request";

export async function POST(req: Request) {
  const { data, error } = await getAndValidateRequestData(
    req,
    turnkeyGetWalletAddressRequestSchema
  );
  ...
  try {
    ...
    if (!wallets?.length)
      throw new Error(
        `No wallets for organization '${data.organizationId}' found`
      );
    ...
    if (!accounts?.length)
      throw new Error(
        `No accounts for wallet '${walletId}' (${walletName}) found`
      );
